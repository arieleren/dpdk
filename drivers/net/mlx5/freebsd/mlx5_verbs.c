...
mlx5_txq_ibv_obj_new(struct rte_eth_dev *dev, uint16_t idx)
{
	...
	if (mlx5_devx_obj_ops_en(priv->sh) && !priv->sh->tdn)
	 ...
	 * Will use this value on Rx, when creating matching TIR. Only the DevX
	 * object operations build those TIRs, so with Verbs Rx objects there is
